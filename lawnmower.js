//This code is a modified version of the original lawnmower game from
// https://www.firefliesalco.com/lawnmower-game/
//This code attempts to measure the cash gain rate at various upgrade combinations.
//After collecting the data, the game can use it with the getRate function to 
//determine the gain rate without running the lawnmowing simulation

//AM: move this to after the code since there is no window in node
//window.onload = setup;
...
function Area(name, multiplierBuff, initialBuff, baseColor, grownColor, machineColor, unlockPrice, message, value, machineName, hmm){
    ...
    this.upgrades = [
        ...
        new Upgrade("machineSize", sizeBasePrice*initialBuff, mowerSizeMultiplier+multiplierBuff, function(){if(activeField.machineWidth==activeField.machineHeight){activeField.machineWidth++}else{activeField.machineHeight++}activeField.machineX=0;activeField.machineY=0;}, "%w%x%h%", "%name% Size", function(){return activeField.machineHeight < height/tileSizes[activeField.tileSize];}),
...
function tick(field){
    ...
    //if(!currentlyPrestiging){
        //AM: comment this out so we can fast forward ticks
...
function reset(){
    //AM: disable mulch so a reset just gets us back to the beginning
    //mulch += nextMulch;
    money = 0;
    totalMoney = 0;
...
//AM: below here is new content

const data = {};

function runTest(count, duration, repetitions, setup) {
  reset();
  
  //Object.keys(setup).forEach( k => {
  //  fields[0][k] = setup[k];
  //});
  fields[0].growthAmount = setup.growthAmount;
  fields[0].machineSpeed = setup.machineSpeed;
  fields[0].tileSize = setup.tileSize;
  fields[0].machineWidth = setup.machineWidth;
  fields[0].machineHeight = setup.machineHeight;
  fields[0].regenerate();    
  fields[0].machineX = 0;
  fields[0].machineY = 0;
  
  const tickCount = Math.floor(duration / (fields[0].tickRate / 1000));
  
  const startMoney = money;
  for (let j = 0; j < repetitions * tickCount; j++) {
    tick(fields[0]);
  }
  const endMoney = money;
  const deltaMoney = endMoney - startMoney;
  const deltaTime = repetitions * tickCount * (fields[0].tickRate / 1000);
  const rate = deltaMoney / deltaTime;
  //console.log(count, JSON.stringify(setup), 'rate', rate);
  console.log(count, setup.growthAmount, setup.machineSpeed, setup.tileSize, setup.machineWidth, 'rate', rate);
  return rate;
}
...
//iterate over all g, s, z, and 5 w values
while (true) {
  count++;
  const range = 500 / tileSizes[z];
  let w = 1 + Math.floor(wi * range / 5);
  if (w > range) {w = range;}
  const rate = runTest(count, 1000, 1, {
    growthAmount: g, //[4,58]
    machineSpeed: s, //[1,19]
    tileSize: z, //[0,7]
    machineWidth: w, //[1,500/tileSizes[tileSize]]
    machineHeight: w, //[1,500/tileSizes[tileSize]]
  });
  results[[g,s,z,w].join`,`] = rate;
  break;
  if (g < 60) {
    g += 2;
  } else {
...
function getRate(data,g,s,z,w) {
  const wstep = Math.floor((500 / tileSizes[z]) / 5);
  const wl = Math.floor(w / wstep) * wstep + 1;
  const wh = Math.ceil(w / wstep) * wstep + 1;
  if (w === wl) {
    return data[`${g},${s},${z},${w}`];
  } else {
    const lb = data[`${g},${s},${z},${wl}`];
    const ub = data[`${g},${s},${z},${wh}`];
    const f = (w - wl) / (wh - wl);
    return lb + (ub - lb) * f;
  }
}
